...
#define CERTFILE "c_cert.pem" 
#define KEYFILE  "c_key.pem" 
#define CA_CERT  "s-ca_cert.pem" 
...
{ 
    SSL_CTX *ctx; 
 	...
 	char buf[1024];   	struct hostent *host; 
 	struct sockaddr_in addr; 
 	 
 	X509 *serv_cert;  	char *line; 
    ...
    hostname=strings[1];     portnum=strings[2]; 
...
/*--- Create socket and connect to server. --------*/ 
    ...
    host = gethostbyname(hostname); 
 	...
 	bzero(&addr, sizeof(addr));  	addr.sin_family = AF_INET;  	addr.sin_port = htons(atoi(portnum)); 
...
/*---- Create new SSL connection ------------------------*/ 
 	...
 	ssl = SSL_new(ctx);  	/* create new SSL connection state */  	CHK_NULL(ssl);     
 
	 	SSL_set_fd(ssl, sd);  	/* attach the socket descriptor */ 
         
 	err = SSL_connect(ssl) ; 	 	/* perform the connection */  	CHK_SSL(err); 
	 	printf("Connected with %s encryption\n", SSL_get_cipher(ssl)); 
 	 
 
/*--- Print out the certificates. -------------------*/ 
 
    if (SSL_get_verify_result(ssl) == X509_V_OK)         printf("Server verification succeeded.\n"); 
 
 	serv_cert = SSL_get_peer_certificate(ssl); /* get the server's certificate */  	if ( serv_cert == NULL )  	 	printf("No certificates.\n"); 
	 	else 
	 	{ 
    ...
    FILE *fd = 0;     long tam_fich = 0;     int i = 0; 
    int bytes_rec = 0; 
 
    bzero(&buf, 1024 ); 
    bytes_rec = SSL_read(ssl, buf, sizeof(buf)); 
    ...
    if ((fd = fopen(strings[2], "w")) == NULL){          perror("Abriendo fichero local");          exit(1); 
    }     int k = 0;      bytes_rec = 0; 
     while(bytes_rec < tam_fich){ 
         k = SSL_read(ssl, buf, sizeof(buf)); 
 
         if ((k <= 0)|| !strncmp(buf, "KO:" ,3)){              perror("Leyendo respuesta. Abandono");              close(sd);              fclose(fd); 
             SSL_free(ssl);  	 	/* release connection state */ 
             SSL_CTX_free(ctx);              exit(1); 
         } 
         for (i=0; i<k; i++)              if (putc(buf[i],fd)<0){ 
                 perror("Escribiendo en el fichero local. Abandono");                  close(sd);                  fclose(fd); 
                 SSL_free(ssl); 	 	 	/* release connection state */ 
                 SSL_CTX_free(ctx);                  exit(1); 
             } 
         bytes_rec += k; 
